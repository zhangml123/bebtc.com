...
class Oklink_OAuth
{
    ...
    public function getTokens($code, $grantType='authorization_code')
    {
        ...
        curl_setopt($curl, CURLOPT_URL, 'https://Oklink.com/oauth/token');
        curl_setopt($curl, CURLOPT_CAINFO, dirname(__FILE__) . '/ca-Oklink.crt');
        curl_setopt($curl, CURLOPT_RETURNTRANSFER, true);
        curl_setopt($curl, CURLOPT_HTTPHEADER, array('User-Agent: OklinkPHP/v1'));
        ...
        if($response === false) {
            ...
            curl_close($curl);
            throw new Oklink_ConnectionException("Could not get tokens - network error " . $message . " (" . $error . ")");
        }
        if($statusCode !== 200) {
            throw new Oklink_ApiException("Could not get tokens - code " . $statusCode, $statusCode, $response);
        }
        curl_close($curl);
        ...
        try {
            ...
            throw new Oklink_ConnectionException("Could not get tokens - JSON error", $statusCode, $response);
        }
